...
public class GlobalExceptionHandler {
    ...
    public Result exceptionHandler(IllegalArgumentException e,HttpServletRequest request) {
        ...
        List<FieldErrorException.FieldError> fieldErrors = new ArrayList<>();
        fieldErrors.add(new FieldErrorException.FieldError("", e.getMessage()));
    ...
    public Result exceptionHandler(ConstraintViolationException e,HttpServletRequest request) {
        ...
        set.forEach(error -> {
            ...
            fieldErrors.add(new FieldErrorException.FieldError(field.substring(field.indexOf(".")),error.getMessage()));
        });
        return new Result(new FieldErrorException(fieldErrors),request.getRequestURI(),request.getMethod());
    }
    ...
    private Result validationResult(BindingResult result,HttpServletRequest request){
        ...
        List<FieldErrorException.FieldError> fieldErrors = new ArrayList<>();
        result.getFieldErrors().forEach(error -> fieldErrors.add(new FieldErrorException.FieldError(error.getField(),error.getDefaultMessage())));
        return new Result(new FieldErrorException(fieldErrors),request.getRequestURI(),request.getMethod());
    }
}
